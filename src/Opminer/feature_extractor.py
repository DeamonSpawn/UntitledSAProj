...
def label_feats_from_corpus(corp,feature_detector = bag_of_non_stopwords):
    # gets features and assigns label associated for the reviews
...
def classifier_input_from_feats(lfeats,tfeats):# converts from dict to list (for use with filecorpus and movie_reviews)
    train_feats = []
    test_feats = []
    for label, feats in lfeats.items():
        train_feats.extend([(feat,label) for feat in feats])# tweets in training set
    for label, feats in tfeats.items():
        test_feats.extend([(feat,label) for feat in feats])# tweets in test set
    return train_feats, test_feats